@extends('layouts.default', ['crumbroute' => 'sundryloandetail'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> Sundry Details of {{$sundryloans->first()->staff->name_eng}}
        ...
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($sundryloans as $sundryloan)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $sundryloan->id }}"></td>
                        <td>
                            <a href="{{ route('sundryloan-edit',['id' => $sundryloan->id]) }}">{{ $sundryloan->staff->name_eng  }}</a>
                        </td>
                        <td>{{ $sundryloan->sundryType->title }}</td>
                        ...
                        <td>{{ $sundryloan->notes }}</td>

                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn"
                                   data-id="{{ $sundryloan->id}}" href="javascript:void(0)"><i
                ...
                <tr>
                    ...
                    <td colspan="3">Rs. {{$sundryloans->sum('dr_balance')}}</td>
                    <td colspan="3">Rs. {{$sundryloans->sum('cr_balance')}}</td>
                    <td colspan="2">Payable:- Rs {{$sundryloans->sum('cr_balance')-$sundryloans->sum('dr_balance')}}</td>
                </tr>
                </tbody>
