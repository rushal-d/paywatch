@extends('layouts.default', ['crumbroute' => 'appversion'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('appversion.create') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'appversion', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                <div class="col-md-6 col-sm-12 col-xs-12">
                    {{ Form::label('search', 'Search') }}
                    {{ Form::text('search', request('search'), array('class' => 'form-control search-field', 'placeholder' => 'Search...'))}}
                </div>
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('appversion')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All App Version
            <span class="tag tag-pill tag-success pull-right">{{ $appVersions->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($appVersions as $appVersion)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $appVersion->id }}"></td>
                        <td>
                            <a href="{{ route('appversion.edit',['id' => $appVersion->id]) }}">{{ $appVersion->app_version_name  }}</a>
                        </td>
                        <td>{{$appVersion->description}}</td>
                        <td>{{$appVersion->path_name}}</td>
                        {{--<td>--}}
                        ...
                        <td class="actions-col">
                            <div class="actions">
                                <a href="{{route('appversion.downloadZip', ['id' => $appVersion->id])}}" title="Download Zip file" class="btn btn-sm btn-outline-success"><i class="fa fa-download"></i></a>
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $appVersion->id }}"
            ...
            <div class="pagination-links">{{ $appVersions->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('appversion.destroyselected') }}',
