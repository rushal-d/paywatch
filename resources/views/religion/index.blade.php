@extends('layouts.default', ['crumbroute' => 'religion'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('religioncreate') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'religion', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Religion
            <span class="tag tag-pill tag-success pull-right">{{ $religions->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($religions as $religion)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $religion->id }}"></td>
                        <td>
                            <a href="{{ route('religionedit',['id' => $religion->id]) }}">{{ $religion->religion_name  }}</a>
                        </td>
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $religion->id }}"
            ...
            <div class="pagination-links">{{ $religions->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('religiondestroyselected') }}',
