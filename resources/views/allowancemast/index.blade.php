@extends('layouts.default', ['crumbroute' => 'allowance'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card"><div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('allowance-create') }}"><i class="fa fa-plus"></i> Add New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'allowance-index', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('allowance-index')}}"><i class="icon-refresh"></i> Reset</a>
                </div>
            </div>
            {{ Form::close()  }}
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Allowance
            <span class="tag tag-pill tag-success pull-right">{{ $allowances->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($allowances as $allowance)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $allowance->allow_id }}"></td>
                        <td><a href="{{ route('allowance-edit',['id' => $allowance->allow_id]) }}">
                                {{ $allowance->allow_title }}
                            </a></td>
                        <td>{{ $allowance->allow_amt}}</td>
                        <td>{{ $allowance->effect_date_np}}</td>
                        <td>{{ $allowance->allow_code}}</td>
                        <td>
                         <span class="badge badge-{{  ($allowance->status_id == '1') ? 'success' : 'danger' }}">
                                {{ $allowance->status_id=='1'? 'Active':'Deactive' }}
                            </span>
                        </td>
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $allowance->allow_id }}" href="javascript:void(0)"><i class="fa fa-remove"></i></a>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="pagination-links">{{ $allowances->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function(value) {
                        if(value){ //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('allowance-destroy-selected') }}',
