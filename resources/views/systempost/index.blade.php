...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('system-post-create') }}"><i class="fa fa-plus"></i> Add New</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('system-post-order') }}"><i class="fa fa-align-justify"></i> Manage Order</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'system-post', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('system-post')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Designation (Post) Details
            <span class="tag tag-pill tag-success pull-right">{{ $systempost->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                <tr align="left">
                    ...
                    <th width="10%">Designation (Post)</th>
                    <th width="10%">No. of Staffs</th>
                    <th width="10%">Salary</th>
                    <th width="10%">Grade Amount</th>
                    <th width="10%">Effective From Date</th>
                    <th width="10%">Effective To Date</th>
                ...
                @foreach($systempost as $systemposts)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $systemposts->post_id }}"></td>
                        <td>
                            @if($systemposts->active==1)
                                <a href="{{ route('system-post-edit',['id' => $systemposts->post_id]) }}">{{ $systemposts->post_title  }}</a>
                            @else
                                {{ $systemposts->post_title  }}
                            @endif
                        </td>
                        <td> {{ $systemposts->staffs_count }}</td>
                        <td> {{ $systemposts->basic_salary }}</td>
                        <td>
                            Rs.{{ $systemposts->grade_amount }}</td>
                        <td>{{ $systemposts->effect_date_np }}</td>
                        <td>{{ $systemposts->effective_to_date_np }}</td>
                        <td>
                               <span class="badge badge-{{  ($systemposts->active == '1') ? 'success' : 'danger' }}">
                                {{ $systemposts->active=='1'? 'Active':'Deactive' }}
                            </span>
                        </td>
                        <td>
                            {{ $systemposts->created_at }}
                        </td>
                        <td>
                            {{ $systemposts->updated_at }}
                        </td>
                        <td class="actions-col">
                            {{--<div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn"
                                   data-id="{{ $systemposts->post_id }}" href="javascript:void(0)"><i
            ...
            <div class="pagination-links">{{ $systempost->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('system-post-destroy-selected') }}',
