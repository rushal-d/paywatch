@extends('layouts.default', ['crumbroute' => 'sundryType'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('sundry-type-create') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'sundry-type-index', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('sundry-type-index')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> Sundry Types
            <span class="tag tag-pill tag-success pull-right">{{ $sundryTypes->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                <tr align="left">
                    ...
                    <th width="20%">Sundry Type Title</th>
                    <th width="20%">Type</th>
                    <th width="8%">Action</th>
                </tr>

                @foreach($sundryTypes as $sundryType)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $sundryType->id }}"></td>
                        <td>
                            <a href="{{ route('sundry-type-edit',['id' => $sundryType->id]) }}">{{ $sundryType->title  }}</a>
                        </td>
                        <td>{{$types[$sundryType->type]}}</td>
                        <td class="actions-col">
                          {{--  <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $sundryType->id }}"
            ...
            <div class="pagination-links">{{ $sundryTypes->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('sundry-type-destroy-selected') }}',
