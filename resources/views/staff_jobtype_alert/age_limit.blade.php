@extends('layouts.default', ['crumbroute' => 'staff-jobtype-alert-warning-age'])
@section('title', $title)
@section('style')
...
@section('content')
            ...
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <button class="nav-link text-danger btn-link" id="staff-main-warning" type="submit"><i
                            class="fa fa-exclamation-circle"></i> Age Alert ({{$staffmains->total()}})
                    </button>
                </li>
                <li class="nav-item">
                    <button class="nav-link text-primary btn-link change-contract-bulk-button">
                        Change to Bulk Contract
                    </button>
                </li>
            </ul>
        </div>
        {{ Form::open(array('route' => 'staff-job-type-alert.age-limit','method' => 'get', 'id' => 'staff-get-promotion-alert'))  }}
        <div class="card">
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Staff <span class="text-danger">

            [Maximum Age Limit: {{config('constants.staff_above_age_promotion.maximum_age_in_years')}} years]
            </span>
            ...
            <table class="table table-responsive table-striped table-hover table-all">
                ...
                @foreach($staffmains as $staffmain)
                    <tr style="color: red">
                        ...
                        <td>
                            {{ $staffmain->staff_dob}}
                            ({{\App\Helpers\DateHelper::getAgeToday($staffmain->staff_dob)}}
                            years)
                        </td>

                        @if(!empty($staffmain->appo_date))
                            <td>
                                ({{\App\Helpers\DateHelper::getAgeToday($staffmain->appo_date)}} years ago)
                            </td>
                        @else
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Change Above Age Contract</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        {!! Form::open(['method' => 'post', 'route' => 'staff-job-type-alert.change-above-age-contract','id' => 'modal-single-form']) !!}
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::label('salary_modal', 'Salary:',  ['class' => 'control-label']) !!}
                                {!! Form::number('salary_modal', '', ['class' => 'form-control nep-date custom-modal col-md-5', 'data-validation' => 'required','placeholder' => 'Enter a salary']) !!}
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $('#modal-single-form').submit(function (e) {
            ...
            var $_this = $(this);
            var $_salary_input = $('#salary_modal');
            var $_staff_central_id_input = $('#staff_central_id_modal');

            if ($_salary_input.val() !== '' || $_salary_input.val() <= 0) {
                e.preventDefault();
