@extends('layouts.default', ['crumbroute' => 'bonuses.index'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a style="display: inline-block" class="nav-link" href="{{ route('bonuses.create') }}"><i class="fa fa-plus"></i>
                        Add New</a>
                    <a style="display: inline-block" class="nav-link" href="{{route('bonuses.filterview')}}"><i class="fas fa-plus mr-1"></i>Bulk Entry</a>
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'bonuses.index', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                    ...
                    {{ Form::text('date', $_GET['date'] ?? null, array('class' => 'adjust-width form-control date','id'=>'date1' , 'placeholder' => 'Please enter Date', 'readonly')) }}
                ...
                <div class="col-md-12 col-sm-12 col-xs-12 text-right">
                    ...
                    <a class="btn btn-outline-success btn-reset" href="{{ route('bonuses.index')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Approved Bonuses
            <span class="tag tag-pill tag-success pull-right">{{ $bonuses->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($bonuses as $bonus)
                    <tr>
                        <td>
                            <input type="checkbox" class="check-id" data-id="{{ $bonus->id }}">
                        </td>
                        <td>{{$bonus->staff->name_eng}}</td>
                        <td>{{$bonus->staff->id}}</td>
                        <td>{{$bonus->fiscal_year->fiscal_code}}</td>
                        <td>{{$bonus->branch->office_name}}</td>
                        <td>{{$bonus->received_date}}</td>
                        <td>{{$bonus->received_date_np}}</td>
                        <td>{{$bonus->received_amount}}</td>
                        <td>
                            <a href="{{route('bonuses.edit', $bonus->id)}}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                            <a href="javascript:void(0)" data-id="{{$bonus->id}}" class="delete-btn btn btn-sm btn-danger"><i class="fas fa-times"></i></a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            </table>
            <div class="pagination-links">{{ $bonuses->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                   ...
                   callback: function (value){
                       if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{route('staff-bonuses-destroy-selected')}}',
                                data: {
                                    _token: '{{csrf_token()}}',
                                    ids: ids
                                },
