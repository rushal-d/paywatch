@extends('layouts.default', ['crumbroute' => 'department'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('department-create') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'department', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('department')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Department
            <span class="tag tag-pill tag-success pull-right">{{ $departments->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($departments as $department)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $department->id }}"></td>
                        <td>
                            <a href="{{ route('department-edit',['id' => $department->id]) }}">{{ $department->department_name  }}</a>
                        </td>
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $department->id }}"
            ...
            <div class="pagination-links">{{ $departments->appends(
	  			$_GET
	  			)->links()
	  		}}
            </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('department-destroy-selected') }}',
