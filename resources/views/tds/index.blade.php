@extends('layouts.default', ['crumbroute' => 'tds'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card"><div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('system-tds-create') }}"><i class="fa fa-plus"></i> Add New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'system-tds', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('system-tds')}}"><i class="icon-refresh"></i> Reset</a>
                </div>
            </div>
            {{ Form::close()  }}
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Tds
            <span class="tag tag-pill tag-success pull-right">{{ $tdss->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($tdss as $tds)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $tds->id }}"></td>
                        <td><a href="{{ route('system-tds-edit',['id' => $tds->id]) }}">{{ $tds->fiscal->fiscal_code }}</a></td>
                        <td><a href="{{ route('system-tds-edit',['id' => $tds->id]) }}">{{ $tds->slab }}</a></td>
                        <td><a href="{{ route('system-tds-edit',['id' => $tds->id]) }}">{{ $tds_options[$tds->type] }}</a></td>
                        <td>{{ $tds->amount }}</td>
                        <td>{{ $tds->percent }}</td>
                        <td>
                        ...
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $tds->id }}" href="javascript:void(0)"><i class="fa fa-remove"></i></a>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="pagination-links">{{ $tdss->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function(value) {
                        if(value){ //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('system-tds-destroy-selected') }}',
