@extends('layouts.default', ['crumbroute' => 'vehicle-loan-diff-income-create'])
@section('title', $title)
@section('content')
    {{ Form::open(array('route' => 'vehicle-loan-diff-income-save'))  }}
    <div class="row">
        <div class="col-md-7 col-sm-12">
            {{-- Basic Info --}}
            <div class="basic-info card">
                <h5 class="card-header">Vehicle Loan Diff Income Information</h5>
                <div class="card-block">
                    <div class="card-text">
                        <div class="form-group row">
                            <label for="vehicle_loan_id" class="col-3 col-form-label">
                                Vehicle Loan Id
                            </label>
                            <p>{{$vehicleLoan->vehical_id}}</p>
                            <input type="hidden" id="vehicle_loan_id" name="vehicle_loan_id"
                                   value="{{$vehicleLoan->vehical_id}}">
                        </div>
                        <div class="form-group row">
...
@section('script')
    <script>
        $('#fiscal_year_id').on('ready change', function () {
            let $_this = $(this);
            $.ajax({
                url: '{{route('get-previous-vehicle-loan-diff-income-by-filter')}}',
                type: 'POST',
                data: {
                    _token: '{{csrf_token()}}',
                    fiscal_year_id: $_this.val(),
                    vehicle_loan_id: $('#vehicle_loan_id').val()
                },
