@extends('layouts.default', ['crumbroute' => 'socialsecuritytaxstatement'])
@section('title', $title)
@section('content')
    <form action="{{ route('social-security-tax-statement-personal') }}" method="get" enctype="multipart/form-data">
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <span class="nav-link"> <i class="fa fa-align-justify"></i> Social Security Tax Statement</span>
                </li>
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'social-security-tax-statement', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                <div class="col-md-3 col-sm-12 col-xs-12">
                    {{ Form::label('Fiscal Year', 'Fiscal Year') }}
                    {{ Form::select('fiscal_year',$fiscalyear->pluck('fiscal_code','id'), request('fiscal_year'), array('class' => '', 'placeholder' => 'Fiscal Year','required'))}}
                </div>

                <div class="col-md-3 col-sm-12 col-xs-12">
                    {{ Form::label('Branch', 'Branch') }}
                    {!! Form::select('branch_id',$branch,request('branch_id'),['class'=>'','placeholder'=>'Select One']) !!}

                </div>

                <div class="col-md-3 col-sm-12 col-xs-12">
                    {{ Form::label('From', 'From') }}
                    <select id="month" name="from_month" class="input-sm" >
                        <option value="">Select One</option>
                        <option value="4">Shrawan</option>
                ...
                <div class="col-md-3 col-sm-12 col-xs-12">
                    {{ Form::label('To', 'To') }}
                    <select id="month" name="to_month" class="input-sm" >
                        <option value="">Select One</option>
                        <option value="4">Shrawan</option>
                        <option value="5">Bhadra</option>
                        <option value="6">Aswin</option>
                ...
                <table border="1px" width="80%" align="center" class="text-center">
                    ...
                    @foreach($month_names as $month=>$month_name)
                        <tr>
                            ...
                            <td>{{$details[$month]['no_of_staff'] ?? '-'}}</td>
                            <td>{{$details[$month]['tax_amount'] ?? '-'}}</td>
                            <td>{{$details[$month]['total'] ?? '-'}}</td>
                        </tr>
                    @endforeach
                </table>
            </div>
            ...
            render: {
                option: function (item, escape) {
                    return '<div class="suggestions"><div> Name: ' + item.name_eng + '</div>' +
                        '<div> Main ID: ' + item.main_id + '</div></div>';
                }
            },
