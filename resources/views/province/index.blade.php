@extends('layouts.default', ['crumbroute' => 'education'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('educationcreate') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <span class="tag tag-pill tag-success pull-right">{{ $educations->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($educations as $education)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $education->edu_id }}"></td>
                        <td>
                            <a href="{{ route('educationedit',['id' => $education->edu_id]) }}">{{ $education->edu_description  }}</a>
                        </td>
                        {{--<td>--}}
                        ...
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $education->edu_id }}"
            ...
            <div class="pagination-links">{{ $educations->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('educationdestroyselected') }}',
