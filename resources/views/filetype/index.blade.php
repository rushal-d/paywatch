@extends('layouts.default', ['crumbroute' => 'filetype'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('file-typecreate') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'file-type', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('section')}}"><i
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All File Type
            <span class="tag tag-pill tag-success pull-right">{{ $fileTypes->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                <tr align="left">
                    ...
                    <th width="20%">File Section</th>
                    <th width="8%">Action</th>
                </tr>

                @foreach($fileTypes as $fileType)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $fileType->id }}"></td>
                        <td>
                            <a href="{{ route('file-typeedit',['id' => $fileType->id]) }}">{{ $fileType->file_type  }}</a>
                        </td>
                        <td>
                            {{ $fileType->file_section  }}
                        </td>

                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $fileType->id }}"
            ...
            <div class="pagination-links">{{ $fileTypes->appends($_GET)->links()}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('sectiondestroyselected') }}',
