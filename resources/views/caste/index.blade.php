@extends('layouts.default', ['crumbroute' => 'caste'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card">
        <div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('castecreate') }}"><i class="fa fa-plus"></i> Add
                        New</a>
                </li>
                <li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'caste', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All Caste
            <span class="tag tag-pill tag-success pull-right">{{ $castes->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                @foreach($castes as $caste)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $caste->id }}"></td>
                        <td>
                            <a href="{{ route('casteedit',['id' => $caste->id]) }}">{{ $caste->caste_name  }}</a>
                        </td>
                        <td class="actions-col">
                            <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $caste->id }}"
            ...
            <div class="pagination-links">{{ $castes->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function (value) {
                        if (value) { //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('castedestroyselected') }}',
