@extends('layouts.default', ['crumbroute' => 'leave'])
@section('title', $title)
...
@section('content')
    ...
    <div class="card"><div class="quick-actions">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('systemleavecreate') }}"><i class="fa fa-plus"></i> Add New</a>
                </li>
                {{--<li class="nav-item">
        ...
        <div class="search-box">
            {{ Form::open(array('route' => 'systemleave', 'method'=> 'GET' ,'class' => 'search-form')) }}
            <div class="row">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <a class="btn btn-outline-success btn-reset" href="{{ route('systemleave')}}"><i class="icon-refresh"></i> Reset</a>
                </div>
            </div>
            {{ Form::close()  }}
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <i class="fa fa-align-justify"></i> All System Leave
            <span class="tag tag-pill tag-success pull-right">{{ $systemleaves->total() }}</span>
        </div>
        <div class="card-block">
            <table class="table table-responsive table-striped table-hover table-all" width="100%" cellspacing="0">
                ...
                <tr align="left">
                    ...
                    <th width="10%">Leave Description</th>
                    <th width="10%">Leave code</th>
                    <th width="10%">Maximum Days</th>
                    <th width="10%">No. Of Days</th>
                    <th width="10%">Initial Setup</th>
                    <th width="10%">Date Added</th>
                    <th width="10%">Date Modified</th>
                    <th width="8%">Action</th>
                </tr>

                @foreach($systemleaves as $systemleave)
                    <tr>
                        <td><input type="checkbox" class="check-id" data-id="{{ $systemleave->leave_id }}"></td>
                        <td><a href="{{ route('systemleaveedit',['id' => $systemleave->leave_id]) }}">{{ $systemleave->leave_name  }}</a></td>
                        <td>{{$systemleave->leave_code}}</td>
                        <td>{{$systemleave->max_days}}</td>
                        <td>{{$systemleave->no_of_days}}</td>
                        <td>{{($systemleave->initial_setup==1)?'Yes':'No'}}</td>
                        <td>
                            {{ $systemleave->created_at }}
                        </td>
                        <td>
                            {{ $systemleave->updated_at }}
                        </td>
                        <td class="actions-col">
                           {{-- <div class="actions">
                                <a class="btn btn-sm btn-outline-danger delete-btn" data-id="{{ $systemleave->leave_id }}" href="javascript:void(0)"><i class="fa fa-remove"></i></a>
                            </div>--}}
                        </td>
                    </tr>
            ...
            <div class="pagination-links">{{ $systemleaves->appends($_GET)->links()
	  		}}
            </div>
        </div>
    </div>
                ...
                vex.dialog.confirm({
                    ...
                    callback: function(value) {
                        if(value){ //true if clicked on ok
                            $.ajax({
                                type: "POST",
                                url: '{{ route('systemleavedestroyselected') }}',
